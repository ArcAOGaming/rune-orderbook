--- json.lua -- the JSON module every process in this repo actually uses.
---
--- Lifted VERBATIM out of `hyper-aos.lua`, which is where it used to come from.
--- Nothing here is new code: it is the same implementation, registered under
--- the same name, so `require(".json")` keeps working untouched.
---
--- Why it was lifted:
---
...
---
--- The rest of aos was still being compiled into the process and held in the
--- Luerl heap: a handler pipeline, a message router, a 10,000-entry `Inbox`,
--- scheduler and assignment logic, a base64 codec, a pretty-printer and a
--- colour table -- none of which can run, all of which HyperBEAM term_to_binary
--- serialises into every slot snapshot.
---
--- If a process here ever DOES need the aos pipeline, bundle `hyper-aos.lua`
--- instead of this file; it still registers `.json` the same way and the two
--- are interchangeable at that seam.
